 ...
 * {@code CONDITION_EVALUATION_FAILED} failure rather than propagating - but a custom implementation
 * is still expected to honor the contracts documented on each method below, and to redact/omit any
 * secret value it knows about from {@link #describe}'s own text (the engine additionally redacts
 * every currently-known secret out of a stored description as defense in depth, but that is not a
 * substitute for a condition never emitting one intentionally). {@link #referencedVariables} is
 * validated separately and earlier, at {@code Workflow.Builder#build()} - definition time, before
 * any execution exists: a {@code null} result, a result containing {@code null}, or a {@link
 * RuntimeException} from that method is a definition error and throws {@link
