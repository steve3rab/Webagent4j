...
**The declared `maxIterations` bound is deliberately not part of a recording at all.** A `WorkflowExecutionPlan` represents a loop's body once, structurally, and never encodes how many iterations were authorized (see [Workflows](workflow.md#bounded-loops)) - so neither the plan nor `RecordingV2PlanTreeValidator` can check a recorded iteration count against it. That check instead happens only where the live bound is actually available: `ReplayValidator` (see below) rejects a recording whose iteration count for any loop - at any nesting depth - exceeds what the live `Workflow`'s own current definition authorizes for that exact loop, via the existing `Workflow#loopMaxIterations(stepId)` accessor. A hostile recording that is otherwise perfectly self-consistent (a valid plan, a valid tree, valid per-iteration decisions) but claims more iterations than the live loop's declared bound permits is rejected at replay-validation time, not silently accepted because its shape alone looked fine.

**Resolving a nested loop's bound is structural, never by a recorded runtime step ID.** A nested loop's *recorded* step ID carries a runtime iteration qualification (for example `inner#0`, composing further for a loop nested inside a loop), which never equals any *declared* `WorkflowStepId` in the live workflow - `Workflow#loopMaxIterations` looks up a loop by its plain declared ID, exactly as `Workflow.Builder` assigned it. `ReplayValidator` resolves this correctly at any nesting depth by walking the recorded plan (already proven, by the time this check runs, to be structurally identical to `WorkflowPlanner.plan(workflow)`) in lockstep with the recorded execution tree, position by position - a recorded plan node's `stepId()` is never runtime-qualified, since `WorkflowPlanner` builds it directly from a step's own declared ID at every depth - and reads `maxIterations` from the live workflow using that positionally-aligned declared ID. This is why every occurrence of the same nested loop declaration, across however many outer iterations it actually ran under, is checked against the identical bound: there is exactly one plan node for it, reused at every position it appears. Nothing here strips, parses, or heuristically recovers a declared ID from a runtime-qualified one - a declared ID that itself happens to contain a literal `#`-digit sequence is resolved correctly, unaffected. If a recorded `LOOP` node's positionally-aligned live loop declaration cannot be resolved at all - structurally unreachable once plan equality already holds, but never assumed - replay validation fails closed (`INCOMPATIBLE_WORKFLOW`) rather than treating that loop as unbounded.

## Bounded parallelism

Deterministic Bounded Workflow Parallelism (`WorkflowSteps.parallel`, see [Workflows](workflow.md#bounded-parallelism)) is recorded strictly additively too: a recorded `WorkflowStepType.PARALLEL` node's children are its own `WorkflowStepType.PARALLEL_BRANCH` nodes, one per declared branch, always in **branch-definition order** - never the order branches actually finished, since a recording captures a deterministic logical trace, not a wall-clock one.

**A `PARALLEL_BRANCH`'s own step ID is structural, not runtime-generated.** Each branch's recorded step ID is qualified `parallelStepId + idSuffix + "@" + branchIndex`, mirroring exactly how `WorkflowEngine.Session#executeParallelStepInto` composes a real branch's own step IDs (composing with a loop's own `"#" + iteration` suffix when a `PARALLEL` step sits inside a loop body, or vice versa) - checked positionally against the plan, never recovered by parsing the runtime ID itself, the same discipline already established for a loop iteration's own qualified ID (see [Bounded loops](#bounded-loops) above).

**A `PARALLEL` node's own status governs whether it may carry children at all.** A non-`SUCCEEDED` `PARALLEL` node (skipped by its own optional guard, failed before any branch launched, or never reached) must carry zero children - no branch is ever launched in any of those cases. A `SUCCEEDED` node must carry exactly one `PARALLEL_BRANCH` child per branch the plan declares, positionally matched - never a different count.

**A `PARALLEL_BRANCH`'s own status is either `SUCCEEDED` or `NOT_RUN` - never anything else.** `SUCCEEDED` means the branch was launched; its own children carry its real outcome (success or failure), matched positionally against that branch's own plan exactly like any other level, and are never empty (a declared branch is never empty, since `WorkflowSteps.parallel` rejects one outright). `NOT_RUN` means the branch was never launched - either an earlier-declared sibling branch's failure made the overall step's result final before this branch's own outcome could be authoritatively included, or the whole step never started - and always carries zero children, regardless of what that branch may have actually computed in the background before being discarded: a `PARALLEL` branch is never permitted an observable side effect in the first place, so there is nothing unsafe about a recording simply not retaining a discarded branch's already-computed read.

**Cross-branch collisions and nesting depth are the same checks as everywhere else, applied per branch.** `RecordingV2PlanTreeValidator`'s existing per-level output/type/secret-classification and nesting-depth checks apply identically inside each `PARALLEL_BRANCH`'s own recorded content; a `PARALLEL` step's own transition into its branches consumes one nesting level (like a `LOOP`'s transition into its iterations), while a `PARALLEL_BRANCH`'s own transition into its branch's content does not (the level is already consumed at the `PARALLEL` step itself) - the identical bookkeeping `JsonWorkflowRecordingV2Codec` already applies for `LOOP`/`LOOP_ITERATION`.

**A loop nested inside a `PARALLEL` branch resolves its bound exactly like any other nested loop.** `ReplayValidator`'s structural, plan-based bound resolution (see [Bounded loops](#bounded-loops) above) recurses into every one of a `PARALLEL` node's own branches - not just one selected branch, since every declared `PARALLEL` branch structurally always runs - so a loop nested at any depth inside any branch has its `maxIterations` bound checked against the live workflow's own declaration, using the same positionally-aligned plan node, never the recorded tree's own runtime-qualified step ID.
...
A recording is eligible for replay only once **two** independent guarantees both hold - matching the recorded plan is never sufficient on its own, since a plan match alone says nothing about whether the recorded tree paired with it is genuine. Put another way, a `WorkflowRecordingV2` is treated as hostile, stale, or fabricated input at three distinct levels, never conflated: (a) **self-consistency** - is the recording coherent with itself (guarantee 1 below); (b) **compatibility with the live `Workflow`** - is it still authorized by the current definition (guarantee 2 below); (c) **execution authorization** - a recording, even one that passes both (a) and (b), is never treated as current authorization for a side effect (see [No live side-effect replay](#no-live-side-effect-replay)), which is exactly why this initial Deterministic Replay implementation performs no side effect at all.
