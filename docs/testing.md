...
This Java 21 CI pin verifies the minimum supported version; it is not a maximum-version cap. Maven
Enforcer accepts JDK 21 and all later feature releases (`[21,)`), while compilation continues to use
`--release 21` so the generated bytecode remains usable on Java 21.
