# WebAgent4J documentation

WebAgent4J is a Java 21 foundation for deterministic web automation. Start with
[Getting started](getting-started.md), then use the focused guides below.

- [Architecture and dependency rules](architecture.md)
- [Module graph](modules.md)
- [Browser lifecycle](browser.md)
- [Semantic locators](locators.md)
- [Observations](observation.md)
- [Actions](actions.md) and [verification](verification.md)
- [Testing](testing.md) and [contributing](contribution.md)
- [Roadmap](roadmap.md)

Extraction, crawling, workflows, recording, and plugins currently have documented boundaries rather
than incomplete public implementations. Their design notes are explicit about current status.
