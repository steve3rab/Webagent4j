...
- Deterministic `if`/`else` branching (`WorkflowSteps.ifElse`/`ifThen`) is supported - a condition evaluated exactly once selecting exactly one of two step sequences, nested conditional branching is supported up to the framework's 64-level combined control-flow nesting limit (each branch measured independently, never summed) - see [workflow.md#branching](workflow.md#branching). A definition exceeding that limit is rejected at build time with a controlled error, never a `StackOverflowError`. It is a narrow control-flow primitive, not a general rules/expression DSL.
- Bounded, explicitly-limited repetition (`WorkflowSteps.loop`) is supported - a mandatory `maxIterations` bound checked against a framework-wide maximum, a continuation condition evaluated exactly once per iteration attempt, and fail-closed behavior (`LOOP_ITERATION_LIMIT_EXCEEDED`) if the condition is still true once the bound is reached - see [workflow.md#bounded-loops](workflow.md#bounded-loops). It shares the same combined control-flow nesting limit as conditional branching, plus a separate cumulative executed-step-node budget across the whole execution to guard against a nested-loop structure that is locally bounded yet combinatorially explosive. There is no unbounded loop, no `while(true)`-style construct, and no arbitrary mutable state carried across iterations beyond what an ordinary step output already allows (never treated as definitely available afterward, since a loop may run zero iterations).
- Bounded, read-only/observational concurrency (`WorkflowSteps.parallel`) is supported - two to eight concurrent branches, joined deterministically in branch-definition order regardless of real completion order. A Workflow `ACTION` step is never permitted inside a `PARALLEL` branch, unconditionally - there is no caller-declarable escape hatch, since this framework has no way to mechanically verify that an arbitrary caller-supplied `IWorkflowActionFactory`'s prepared action never mutates page state, navigates, or performs any other observable side effect - see [workflow.md#bounded-parallelism](workflow.md#bounded-parallelism). A `PARALLEL` branch performing a governed browser side effect (a click, a type, a navigation, a mutation) is explicitly out of scope for this version, reserved for a future, separate capability with its own, real safety mechanism. This framework also makes no claim that arbitrary caller Java code is mechanically pure: a caller-supplied `IWorkflowCondition` remains a trusted extension point subject to its own existing side-effect-free contract, exactly as it already is outside `PARALLEL`.
- **Caller interruption while joining a `PARALLEL` step's branches is cooperative, not forced.** `Future.cancel(true)` is a request the JVM has no safe way to turn into a forced kill: a branch task that deliberately ignores its own interruption may still be running on its own daemon worker thread after `WorkflowEngine#execute`/`#executeWithTree` has already returned. The engine guarantees it never waits for such a task indefinitely (a fixed, internal, non-configurable grace period bounds how long it waits for the executor to shut down) and never observes that task's eventual result again once it has moved on - but it does not, and cannot, guarantee every branch thread has actually terminated by the time it returns. A session-wide signal (checked at every `CONDITIONAL`/`LOOP`/`PARALLEL` interruption boundary, in addition to the physical interrupt flag) narrows, but does not eliminate, the case where a hostile branch's own code clears its own interrupt flag - see [workflow.md#bounded-parallelism](workflow.md#bounded-parallelism) for the full contract.
- No recursion over data, DAG scheduler, fork/join beyond `parallel`'s own bounded join, `switch`/`case`, workflow variables reassignable across branches, transactions/sagas, persistence, checkpoint/resume, scheduling, cron, external event triggers, or YAML/JSON workflow language.
...
- `new WorkflowIntrospector().inspect(workflow)` returns a `WorkflowIntrospectionReport` - see [workflow.md#static-workflow-introspection](workflow.md#static-workflow-introspection). It is a purely static complexity/safety-surface summary computed from an already-valid definition's own structure: it never evaluates a condition (not even `IWorkflowCondition#referencedVariables()`), never invokes an `IWorkflowActionFactory`, never touches a backend/browser/network resource, and never creates a thread - a `PARALLEL` step's branches are inspected sequentially. `maximumPotentialExecutionNodes()` is a conservative, saturating-arithmetic upper bound on how many entries a single execution's flat result could contain, never a prediction of what an execution will actually do: a loop that always exits after its first real iteration is indistinguishable here from one that always runs to its declared bound. `mayExceedRuntimeNodeBudget()` exceeding this engine's own node budget is information for a caller's own policy, never a validation failure - a large static potential does not make a workflow invalid, and this is never folded into `WorkflowValidationReport`. `riskIndicators()` lists small, named structural facts, never a combined numeric score. There is no timing, memory, CPU, or network-bandwidth estimate, and no probabilistic guess at which branch a condition will select at runtime.
...
- Recording V2 ([recording.md#recording-v2](recording.md#recording-v2)) captures the executed workflow's `WorkflowExecutionPlan` and a tree mirroring `WorkflowExecutionTree`, with a typed `WorkflowPlanOutput` (name, type, secret classification) per published output instead of V1's bare output-variable name, including a `PARALLEL` step's own branches (`PARALLEL_BRANCH` entries, always in branch-definition order - never the order branches actually finished). There is currently no published JSON Schema file for V2 (unlike V1's); the Java model and codec are the sole authoritative description. Recording V1 has no `PARALLEL`/`LOOP` support at all and is not retrofitted to gain any.
