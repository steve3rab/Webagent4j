...
- Bounded, read-only/observational concurrency (`WorkflowSteps.parallel`) is supported - two to eight concurrent branches, joined deterministically in branch-definition order regardless of real completion order, never treating an `ACTION` step as safe to run concurrently unless its factory explicitly opts in via `IWorkflowActionFactory#isParallelSafe()` - see [workflow.md#bounded-parallelism](workflow.md#bounded-parallelism). A `PARALLEL` branch performing a governed browser side effect (a click, a type, a navigation, a mutation) is explicitly out of scope for this version, reserved for a future, separate capability. This framework makes no claim, of its own, about whether a specific caller-supplied action factory's underlying resource (an `IPage`, for example) actually tolerates concurrent access from two branches' own dedicated threads - live browser objects are not generally thread-safe (see "Browser and frame boundaries" above); declaring a factory `isParallelSafe()` is the caller's own assertion and responsibility, not a framework-verified guarantee.
