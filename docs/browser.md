...
The Playwright adapter implements Chromium, Firefox, and WebKit launch paths. Nightly and release qualification infrastructure now runs the same deterministic robustness corpus against all three engines through this same public browser path, but only Chromium has actually been observed passing it; Firefox and WebKit are not yet release-gated. See [Support matrix](support-matrix.md#browser-backend) before treating engine availability as equal qualification.
